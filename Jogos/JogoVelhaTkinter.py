...
# Definindo janela e elementos na tela
global count
root= Tk()
root.configure(bg="black")
root.title("TIC TAC TOE - Jogo da Velha")
Label(text="Jogo da velha",fg="white",bd=5,bg="black").grid(row=0,column=0,columnspan=3)
count=0
p1=Label(text="X : Jogador 1 =",bg="black",fg="white").grid(row=1,column=0)
pn1=Entry(root,fg="black",width=15,bd=4)
pn1.grid(row=1,column=1)
p2=Label(text="O : Jogador 2 =",bg="black",fg="white").grid(row=2,column=0)
...
def check():
	...
	if count>=5:
		if l[1]==l[4] and l[4]==l[7]:
			x=l[1]
			if x=="X":
				messagebox.showinfo("Game Over",n1+" Venceu o jogo!")
				flag=1
				sys.exit()
			if x=="O":
				messagebox.showinfo("Game Over",n2+" Venceu o jogo!")
				flag=1
				sys.exit()
...
def start():
	...
	b8['state']=NORMAL
	b9['state']=NORMAL
st=Button(text="Clique para iniciar",command=readname,state=NORMAL)
